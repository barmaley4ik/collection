<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('nominations')">Номинации</inertia-link>
          ...
          <div class="flex w-full flex-col pb-8 pr-6"  v-show="lang == current_locale" :key=" 'text_day' + lang "> 
            <label class="form-label">Текст:</label>
              ...
              <div class="w-full"  v-bind:class="(defaultlocale == lang) ? '' :'flex'">
                <editor :name="'text_day[' + lang + ']'" v-model="form.text_day[lang]" :init="{plugins: 'autolink lists hr anchor searchreplace visualblocks code insertdatetime media paste', height: 400, language: 'ru', skin: 'oxide', branding: false, menubar: 'format', toolbar: 'styleselect | fontsizeselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist | code '  }" />
...
export default {
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        text_day: JSON.parse(this.nomination.text_day),
        day: this.nomination.day,
        festival_id: this.nomination.festival_id,
      },
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('nominations.update', this.nomination.id), this.form)

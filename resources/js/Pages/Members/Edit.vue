    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <div class="flex w-full" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'mr-6'"> 
...
export default {
  metaInfo: { title: 'Редактировать участника' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        name: JSON.parse(this.member.name),
        slug: this.member.slug,
        place_number: this.member.place_number,
        title: JSON.parse(this.member.title),
        description: JSON.parse(this.member.description),
        studio: JSON.parse(this.member.studio),
        social: JSON.parse(this.member.social),
        portfolio: this.member.portfolio == '[null]' ? ["",] :JSON.parse(this.member.portfolio),
        city: JSON.parse(this.member.city),
        logo: this.member.logo,
        country_id: this.member.country_id,
        festival_id: this.member.festival_id,
        name_ru: this.member.name_ru,
      },
      imageField: '',
      imageFieldIndex: '',
      error_msg:{},
...
/*       var lengthKey = this.form.portfolio.length;
      console.log('dlina:'+ lengthKey);    */   
      if (v.length>0) {
    ...
    submit() {
      this.sending = true
      this.$inertia.put(this.route('members.update', this.member.id), this.form)
    ...
    selectFile(fName, fIndex="") {
     // console.log('fname=' + fName+ ' findex='+fIndex + ' findexlenght=' + fIndex.length)
      if (fIndex !=="") {

<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('biz-proposals')">Заявка на торговое место</inertia-link>
    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <select-input v-model="form.specialization_id" :errors="$page.errors.specialization_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Формат участия">
          ...
          <textarea-input v-model="form.product" :errors="$page.errors.product" class="pb-8 w-full" label="Описание продукции"/>
          </div>  
          ...
          <text-input v-model="form.illumination" :errors="$page.errors.illumination" class="pr-6 pb-8 w-full lg:w-1/2" label="Дополнительное освещение, Вт" />
          <text-input v-model="form.place_number" :errors="$page.errors.place_number" class="pr-6 pb-8 w-full lg:w-1/2" label="Номер места"/> 
          <select-input v-model="form.place_format_id" :errors="$page.errors.place_format_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Место для торговли">
          ...
          <div class="flex flex-col w-full lg:w-1/2 pr-6 pb-8" >
          <label class="form-label" for="fitting_room">Нужна примерочная?</label>
            <div> 
            <toggle-button v-model="form.fitting_room"
              id="fitting_room"
              name="form.fitting_room"
...
<!--       `portfolio`,    -->   
          <div class="flex flex-col w-full pb-6">
            <template v-for="(item, index) in form.fotos">
            ...
            <div class="flex flex-col w-4/7 mr-4">
              <div class="pr-2 text-md">Фотографии:</div>
            ...
            <button title="Добавить фотографию" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.prevent="addPortfolio(index)">
...
export default {
  metaInfo: { title: 'Изменить заявку на торговое место' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
...
/*   directives: { VueMaskDirective},   */
  ...
  data() {
    return {
      ...
      form: {
        festival_id: this.biz_proposal.festival_id,
        language_id: this.biz_proposal.language_id,
        member: JSON.parse(this.biz_proposal.member),
        specialization_id: this.biz_proposal.specialization_id,
        social: JSON.parse(this.biz_proposal.social),
        place_format_id: this.biz_proposal.place_format_id,
        place_number: this.biz_proposal.place_number,
        product: this.biz_proposal.product,
        fitting_room: this.biz_proposal.fitting_room!=0 ?true :false,
        illumination: this.biz_proposal.illumination,
        phone: this.biz_proposal.phone,
        fotos: JSON.parse(this.biz_proposal.fotos),
        count_worker: (this.biz_proposal.count_worker).toString(),
        comment: this.biz_proposal.comment,
        status_id: this.biz_proposal.status_id,
      },
      statuses: [],
  ...
  methods: {
    ...
    addPortfolio (index){
      ...
      this.imageField='fotos'; 
      this.open = true; 
    },
    addItem(){
      var k, v = 0;
      for (let [key, value] of Object.entries(this.form.fotos)) {
        k = key;
        v = value;   
      }
      if (v.length>0) {
      this.form.fotos = Object.assign({}, this.form.fotos, {
        [++k]: '',
      })      
      }
    ...
    deleteItem(index){
      var lengthKey = (this.form.fotos.length || Object.keys(this.form.fotos).length);
      //console.log('lengthKey='+lengthKey, 'index='+index);
      if (lengthKey>1){
      Vue.delete(this.form.fotos, index)  
      //this.form.portfolio.delete(index);
      } else {
       // this.form.portfolio[index]= '';
        Vue.set( this.form.fotos, index, '' )
      }
    },
    ...
    submit() {
      this.sending = true
      this.$inertia.put(this.route('biz-proposals.update', this.biz_proposal.id), this.form)

<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <span class="text-indigo-400">Настройки</span>
      <span class="text-indigo-400 font-medium">/</span>
      Редактирование
    </h1>
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      <div class="bg-white border-none w-full px-8 text-md ml-1 flex justify-end items-center" >
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <text-input v-model="form.social.facebook" :errors="$page.errors.social" class="pr-6 pb-8 w-full" label="Facebook" />
          <text-input v-model="form.social.instagram" :errors="$page.errors.social" class="pr-6 pb-8 w-full" label="Instagram" />
          <text-input v-model="form.social.youtube" :errors="$page.errors.social" class="pr-6 pb-8 w-full" label="Youtube" />
          <textarea-input v-model="form.google_maps_api" class="pr-6 pb-8 w-full" label="Ключ Google Maps"/>
          <textarea-input v-model="form.google_analytics" class="pr-6 pb-8 w-full" label="Код Google Analytics"/>
          ...
          <div class="flex w-full pb-6">
            ...
            <div class="flex flex-col w-4/7 mr-4">
              <div class="pr-2 text-md">Логотип:</div>
            ...
            <button title="Выбрать Логотип" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true; imageField='logo'">
          ...
          <div class="flex w-full pb-6">
            <div class="flex w-1/6 mr-2 justify-center items-center">
              <img :src="form.footer_logo ? form.footer_logo : noImage" v-bind:class="form.footer_logo ? 'h-16 object-contain bg-gray-400' : 'h-22 object-contain'" >
            </div>
            <div class="flex flex-col w-4/7 mr-4">
              <div class="pr-2 text-md">Логотип (футер):</div>
              <div class="pt-2 text-sm w-5/7 break-all mx-4">{{form.footer_logo ? form.footer_logo : 'Ничего не выбрано'}}</div>
              <button title="Удалить" v-bind:class="form.footer_logo ? 'pt-2 px-4 w-0' : 'invisible'"
              @click.prevent ="form.footer_logo=''">
                  <icon name="trash" class="block w-4 h-4 fill-red-600" />
              </button>
            </div>
            <button title="Выбрать Логотип (футер)" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true; imageField='footer_logo'">
              <svg class="fill-current text-white w-6 h-6 ml-2 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M0 4c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm11 9l-3-3-6 6h16l-5-5-2 2zm4-4a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path></svg>
              ...
            </button>
          </div>
          <div class="flex w-full pb-6">
            <div class="flex w-1/6 mr-2 justify-center items-center">
              <img :src="form.footer_logo_second ? form.footer_logo_second : noImage" v-bind:class="form.footer_logo_second ? 'h-16 object-contain bg-gray-400' : 'h-22 object-contain'" >
            </div>
            <div class="flex flex-col w-4/7 mr-4">
              <div class="pr-2 text-md">Логотип Ассоциации:</div>
              <div class="pt-2 text-sm w-5/7 break-all mx-4">{{form.footer_logo_second ? form.footer_logo_second : 'Ничего не выбрано'}}</div>
              <button title="Удалить" v-bind:class="form.footer_logo_second ? 'pt-2 px-4 w-0' : 'invisible'"
              @click.prevent ="form.footer_logo_second=''">
                  <icon name="trash" class="block w-4 h-4 fill-red-600" />
              </button>
            </div>
            <button title="Выбрать Логотип Ассоциации" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true; imageField='footer_logo_second'">
              <svg class="fill-current text-white w-6 h-6 ml-2 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M0 4c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm11 9l-3-3-6 6h16l-5-5-2 2zm4-4a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path></svg>
              ...
            </button>
          </div>
          <div class="flex w-full pb-6">
            <div class="flex w-1/6 mr-2 justify-center items-center">
              <img :src="form.favicon ? form.favicon : noImage" v-bind:class="form.favicon ? 'h-16 object-contain bg-gray-400' : 'h-22 object-contain'" >
            </div>
            <div class="flex flex-col w-4/7 mr-4">
              <div class="pr-2 text-md">Favicon:</div>
              <div class="pt-2 text-sm w-5/7 break-all mx-4">{{form.favicon ? form.favicon : 'Ничего не выбрано'}}</div>
              <button title="Удалить" v-bind:class="form.favicon ? 'pt-2 px-4 w-0' : 'invisible'"
              @click.prevent ="form.favicon=''">
                  <icon name="trash" class="block w-4 h-4 fill-red-600" />
              </button>
            </div>
            <button title="Выбрать Favicon" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true; imageField='favicon' ">
              <svg class="fill-current text-white w-6 h-6 ml-2 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M0 4c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm11 9l-3-3-6 6h16l-5-5-2 2zm4-4a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path></svg>
              ...
            </button>
          </div>                                  
...
export default {
  metaInfo: { title: 'Редактировать настройки' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        name: JSON.parse(this.setting.name),
        social: JSON.parse(this.setting.social),
        logo: this.setting.logo,
        footer_logo: this.setting.footer_logo,
        footer_logo_second: this.setting.footer_logo_second,
        favicon: this.setting.favicon,
        google_maps_api: this.setting.google_maps_api,
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('settings.update', this.setting.id), this.form)

<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('placeformats')">Форматы мест</inertia-link>
...
export default {
  metaInfo: { title: 'Изменить формат места' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        name: JSON.parse(this.placeformat.name),
        type: this.placeformat.type!=0 ?true :false,
      },
      error_msg:{},
     }
  },
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('placeformats.update', this.placeformat.id), this.form)

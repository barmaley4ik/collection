      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <div class="pr-6 pb-8 w-full lg:w-1/2 items-center mb-6">
            ...
            <input v-model="form.is_default" class="pl-6 w-6 h-6 mr-2 form-checkbox focus:border-red-300 focus:bg-red-400 focus:text-red-500 text-red-500" type="checkbox">
          </div>          
          <div class="pr-6 pb-8 w-full lg:w-1/2 items-center mb-6">
            <label class="form-label">
              Статус:
          ...
          <text-input v-model.lazy="form.phones[index]" :errors="$page.errors.phones" class="pr-6 pb-8 w-full lg:w-1/2" :label="`Телефон`+ (Number([index])+1)" v-mask="'+38(###) ####-###'" placeholder="+38(067) 1234-567" :key="'phone-' + index"/> 
          </template>
...
export default {
  ...
  created() {
    ...
    this.langs.forEach((item, i, langs)=> {
      ...
      Vue.set( this.form.custom_text,item, '');
      Vue.set( this.form.city,item, '');
      Vue.set( this.form.meta_title,item, '');
      Vue.set( this.form.meta_description,item, '');
      Vue.set( this.form.supervisor_project.name,item, '');
      Vue.set( this.form.supervisor_sponsors.name,item, '');
      Vue.set( this.form.supervisor_members.name,item, '');
...
/*     phoneChange: _.debounce(function(index, e){
    ...
    submit() {
      this.sending = true
      this.$inertia.post(this.route('festivals.store'), this.form)

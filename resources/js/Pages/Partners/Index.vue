<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Партнеры</h1>
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('partners.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
      ...
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="(partner, index) in partners.data" :key="partner.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('partners.edit', partner.id)" tabindex="-1">
              <icon name="edit" class="block w-6 h-6 fill-green-600" />
            </inertia-link>
            <button
              class="px-4"
              @click.prevent ="showModal(partner.id)">
...
export default {
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('partners', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('partners.destroy', this.record_id))
    },    
  },
}
</script>

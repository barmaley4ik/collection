<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('partners')">Партнеры</inertia-link>
    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <div class="flex w-full" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'"> 
            <text-input v-model="form.name[lang]" :errors="error_msg['name.'+lang]" class="pb-8 pr-6 w-full" label="ФИО"  @input="changeSlug(lang)"/>
          ...
          <div class="flex w-full" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'"> 
            <text-input v-model="form.meta_title[lang]" :errors="error_msg['title.'+lang]" class="pb-8 pr-6 w-full" label="Title" />
          ...
          <div class="flex w-full" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'">  
          <textarea-input v-model="form.meta_description[lang]" :errors="$page.errors.description" class="pr-6 pb-8 w-full" label="Description"/>
          ...
          <label class="form-label">Фестивали:</label>
          <multiselect 
            v-model="form.festivals" 
            placeholder="Выберите один или несколько Фестивалей" 
            select-label="Выбрать"
            selected-label="Уже выбрано"
            deselect-label="Отменить"
            label="name" 
            track-by="id" 
            :options="festivalsArray" 
            :multiple="true" 
            :taggable="true">
          </multiselect>
          </div>            
          <div class="flex w-full pb-6">
            ...
            <button title="Выбрать Логотип" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true">
          ...
          <div class="flex w-full flex-col pb-8 pr-6" v-for="lang of langs" v-show="lang == current_locale" > 
            <label class="form-label">Описание:</label>
              <div class="w-full flex inline-flex">
                <editor :name="'description[' + lang + ']'" v-model="form.description[lang]" :init="{plugins: 'autolink link image lists hr anchor pagebreak searchreplace visualblocks code insertdatetime media nonbreaking table paste', height: 400, language: 'ru', skin: 'oxide', branding: false, menubar: 'format', toolbar: 'styleselect | fontsizeselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist | link image | code | table'  }" />
                <!-- <div v-if="error_msg['name.'+lang].length" class="form-error">{{ error_msg['name.'+lang] }}</div>  -->
...
export default {
  metaInfo: { title: 'Редактировать партнера' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        name: JSON.parse(this.partner.name),
        name_ru: this.partner.name_ru,
        description: JSON.parse(this.partner.description),
        meta_title: JSON.parse(this.partner.meta_title),
        meta_description: JSON.parse(this.partner.meta_description),
  ...
  created() {
    ...
    let l = this.defaultlocale;
    this.festivalsArray = this.festivals.map(function(element){
      var b = {};
        b['name']=  JSON.parse(element.name)[l];
        b['id'] = element.id;

        return b;
      });
    //let l = this.defaultlocale;
    this.form.festivals = this.partner.festivals.map(function(element){
      var b = {};
        b['name']=  JSON.parse(element.name)[l];
        b['id'] = element.id;
  ...
  methods: {
    ...
    submit() {
      this.sending = true
      this.$inertia.put(this.route('partners.update', this.partner.id), this.form)
...
.tox .tox-editor-header {
 ...
 @import "/public/css/vue-multiselect.min.css";	
 .multiselect__tags {
   min-height: 42px;
 }

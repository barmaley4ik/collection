      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <select-input v-model="form.festival_id" class="pr-6 pb-8 w-full lg:w-1/2" :errors="error_msg['festival_id']" label="Фестивали">
            <option v-for="(festival, index) in fests" :key="festival.id" :value="festival.id" >{{ JSON.parse(festival.name)[defaultlocale] }}
...
export default {
  metaInfo: { title: 'Создать условие участия' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.post(this.route('conditions.store'), this.form)

<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Заявки на торговое место</h1>
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('biz-proposals.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
          ...
          <th class="px-2 pt-6 pb-4">Телефон</th>
        ...
        <tr v-for="(proposal, index) in biz_proposals.data" :key="proposal.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('biz-proposals.edit', proposal.id)" tabindex="-1">
        ...
        <tr v-if="biz_proposals.data.length === 0">
          <td class="border-t px-6 py-4" colspan="8">Нет записей</td>
        </tr>
      </table>
    </div>
    <pagination :links="biz_proposals.links" />
...
export default {
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('biz-proposals', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('biz-proposals.destroy', this.record_id))
    },    
  },
}
</script>

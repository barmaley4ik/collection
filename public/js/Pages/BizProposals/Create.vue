...
export default {
  metaInfo: { title: 'Создать заявку на торговое место' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
...
/*   directives: { VueMaskDirective},   */
  ...
  data() {
    return {
      ...
      form: {
        festival_id: this.festivals.length> 0 ? this.festivals[0].id : null,
  ...
  methods: {
    ...
    submit() {
      this.sending = true
      this.$inertia.post(this.route('biz-proposals.store'), this.form)

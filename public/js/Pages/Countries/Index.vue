<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Страны</h1>
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('countries.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="(сountry, index) in countries.data" :key="сountry.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('countries.edit', сountry.id)" tabindex="-1">
              <icon name="edit" class="block w-6 h-6 fill-green-600" />
            </inertia-link>
            <button
              class="px-4"
              @click.prevent ="showModal(сountry.id)">
...
export default {
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('countries', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('countries.destroy', this.record_id))
    },    
  },
}
</script>

<template>
  <div>
    ...
    <div class="mb-6 flex justify-end items-center">
      ...
      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('placeformats.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="(placeformat, index) in placeformats.data" :key="placeformat.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t px-6 py-4 w-2">
          {{index +1}}
          </td>
          <td class="border-t " colspan="3">
              {{ JSON.parse(placeformat.name)[defaultlocale] }}
          </td> 
                              
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('placeformats.edit', placeformat.id)" tabindex="-1">
              <icon name="edit" class="block w-6 h-6 fill-green-600" />
            </inertia-link>
            <button
              class="px-4"
              @click.prevent ="showModal(placeformat.id)">
                  <icon name="trash" class="block w-6 h-6 fill-red-600" />
            </button>
          </td>          
        </tr>
        <tr v-if="placeformats.data.length === 0">
          <td class="border-t px-6 py-4" colspan="5">Нет записей</td>
        </tr>
      </table>
    </div>
    <pagination :links="placeformats.links" />
...
export default {
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('placeformats', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('placeformats.destroy', this.record_id))
    },    
  },
}
</script>

<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('sliders')">Слайдеры</inertia-link>
      <span class="text-indigo-400 font-medium" v-html="'/'+form.text_slider[defaultlocale]"></span>
    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <div class="flex w-full flex-col pb-8 pr-6" v-show="lang == current_locale" :key=" 'text_slider' + lang "> 
            <label class="form-label">Текст:</label>
             
            <div class="w-full"  v-bind:class="(defaultlocale == lang) ? '' :'flex'">
                <editor :name="'text_slider[' + lang + ']'" v-model="form.text_slider[lang]" :init="{plugins: 'autolink lists hr anchor searchreplace visualblocks code insertdatetime paste', language: 'ru', skin: 'oxide', branding: false, menubar: 'format', toolbar: 'styleselect | fontsizeselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist | code '  }" />
                

                <button v-show="lang != defaultlocale" class="bg-red-600 w-10 h-10 rounded ml-10 cursor-pointer" title="Перевести" v-on:click.stop.prevent="translate(defaultlocale, lang, 'form', 'text_slider', form.text_slider[defaultlocale])">
            ...
            <div v-if="error_msg['text.'+lang]" class="form-error">{{ error_msg['text.'+lang][0] }}</div> 
          </div>            
          </template>
          ...
          <text-input v-model="form.sort" :errors="$page.errors.sort" class="pr-6 pb-8 w-full lg:w-1/2" label="Сортировка" />
          <text-input v-model="form.url" :errors="$page.errors.url" class="pr-6 pb-8 w-full lg:w-1/2" label="Ссылка" />

          <div class="pr-6 pb-8 w-full lg:w-1/2 items-center mb-6">
            <label class="form-label">
              Кнопка:
            </label>
            <input v-model="form.is_button" class="pl-6 w-6 h-6 mr-2 form-checkbox focus:border-red-300 focus:bg-red-400 focus:text-red-500 text-red-500" type="checkbox">
          </div>          
          <div class="flex w-full pb-6">
            ...
            <button title="Выбрать Изображение" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" v-on:click.stop.prevent="open = true; imageField='image'">
...
export default {
  metaInfo: { title: 'Редактировать слайдер' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        text_slider: JSON.parse(this.slider.text_slider),
        sort: this.slider.sort,
        image: this.slider.image,
        festival_id: this.slider.festival_id,
        is_button: this.slider.is_button,
        url: this.slider.url,
      },
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('sliders.update', this.slider.id), this.form)

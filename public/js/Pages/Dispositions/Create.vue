...
<!--           <div class="flex w-full lg:w-1/2" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'" :key="'place-' + lang"> 
            <text-input v-model="form.place[lang]" :errors="error_msg['place.'+lang]" class="pb-8 pr-6 w-full" v-bind:class="(defaultlocale == lang) ? '' :'lg:w-8/9'" label="Место" />
            <button v-show="lang != defaultlocale" class="bg-red-600 w-10 h-10 rounded ml-6 mt-6 lg:ml-0 cursor-pointer" title="Перевести" v-on:click.stop.prevent="translate(defaultlocale, lang, 'form', 'place', form.place[defaultlocale])">
          ...
          <div class="flex w-full lg:w-1/2" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'" :key="'members-' + lang"> 
            <text-input v-model="form.members[lang]" :errors="error_msg['members.'+lang]" class="pb-8 pr-6 w-full" v-bind:class="(defaultlocale == lang) ? '' :'lg:w-8/9'" label="Участник"/>
            <button v-show="lang != defaultlocale" class="bg-red-600 w-10 h-10 rounded ml-6 mt-6 lg:ml-0 cursor-pointer" title="Перевести" v-on:click.stop.prevent="translate(defaultlocale, lang, 'form', 'members', form.members[defaultlocale])">
            <svg class="fill-current text-white w-6 h-6 ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M7.41 9l2.24 2.24-.83 2L6 10.4l-3.3 3.3-1.4-1.42L4.58 9l-.88-.88c-.53-.53-1-1.3-1.3-2.12h2.2c.15.28.33.53.51.7l.89.9.88-.88C7.48 6.1 8 4.84 8 4H0V2h5V0h2v2h5v2h-2c0 1.37-.74 3.15-1.7 4.12L7.4 9zm3.84 8L10 20H8l5-12h2l5 12h-2l-1.25-3h-5.5zm.83-2h3.84L14 10.4 12.08 15z"/></svg>              
            </button>
          </div>    -->       
          ...
          <div class="flex w-full lg:w-1/2" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'" :key="'header-' + lang"> 
            <text-input v-model="form.header[lang]" :errors="error_msg['header.'+lang]" class="pb-8 pr-4 w-full" v-bind:class="(defaultlocale == lang) ? '' :'lg:w-8/9'" label="Заголовок (H1)" />
        ...
        <template v-for="lang of langs" >
          <template v-for="(disposition_place, index) of form.disposition_places" >  
            <div class="flex w-full lg:w-1/2" v-show="lang == current_locale" v-bind:class="(defaultlocale == lang) ? '' :'pr-6'" :key="'place-' + lang + '-' + index"> 
              ...
              <text-input v-model="disposition_place.place[lang]" :errors="error_msg['place.'+lang]" class="pl-4 pb-8 pr-4 w-full" v-bind:class="(defaultlocale == lang) ? '' :'lg:w-8/9'" label="Место" />
            ...
            <div class="flex w-full pr-6 lg:w-1/2" v-show="lang == current_locale" :key="'members-' + lang + '-' + index"> 
              <text-input v-model="disposition_place.member[lang]" :errors="error_msg['member.'+lang]" class="pb-8 pr-4 lg:w-7/8" label="Участник"/>
...
export default {
  metaInfo: { title: 'Создать схему размещения' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  methods: {
    ...
    submit() {
      this.sending = true
      this.$inertia.post(this.route('dispositions.store'), this.form)

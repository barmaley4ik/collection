<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('translations')">Переводы</inertia-link>
      <span class="text-indigo-400 font-medium">/</span>
      {{translation.text[defaultlocale] | truncate(35, '...')}}
    </h1>
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      ...
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <textarea-input v-model="form.text[lang]" :errors="error_msg['text.'+lang]"  class="pr-6 pb-8 w-full" label="Перевод"/>
            <button v-show="lang != defaultlocale" class="bg-red-600 w-10 h-10 rounded ml-6 mt-6 lg:ml-0 cursor-pointer" title="Перевести" v-on:click.stop.prevent="translate(defaultlocale, lang, 'form', 'text', form.text[defaultlocale])">
          ...
          <text-input v-model="form.group" :errors="$page.errors.group" class="pr-6 pb-8 w-full lg:w-1/2" label="Группа" />
          <text-input v-model="form.key" :errors="$page.errors.key" class="pr-6 pb-8 w-full lg:w-1/2" label="Ключ" />
        </div>                       
...
export default {
  metaInfo: { title: 'Создать перевод' },
  layout: Layout,
  components: {
    LoadingButton,
    TextInput,
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('translations.update', this.translation.id), this.form)

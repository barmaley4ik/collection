<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Переводы</h1>
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('translations.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
          ...
          <th class="px-6 pt-6 pb-4">Группа</th>
          <th class="px-6 pt-6 pb-4">Ключ</th>
        ...
        <tr v-for="(translation, index) in translations.data" :key="translation.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t"  colspan="3">
            {{translation.text | truncate(35, '...')}}
          </td>                               
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('translations.edit', translation.id)" tabindex="-1">
              <icon name="edit" class="block w-6 h-6 fill-green-600" />
            </inertia-link>
            <button
              class="px-4"
              @click.prevent ="showModal(translation.id)">
...
export default {
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('translations', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('translations.destroy', this.record_id))
    },    
  },
}
</script>

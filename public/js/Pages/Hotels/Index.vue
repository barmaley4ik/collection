<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Места проживания</h1>
    <div class="mb-6 flex justify-end items-center">

      <inertia-link class="btn-indigo text-white hover:bg-red-600 focus:bg-red-600" :href="route('hotels.create')">
        <span>Создать</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="(hotel, index) in hotels.data" :key="hotel.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t ">
            <div v-if="hotel.festival">
              {{ JSON.parse(hotel.festival.name)[defaultlocale] }}
            </div>
          </td> 
                              
          <td class="flex pt-3 justify-end border-t">
            <inertia-link class="px-4" :href="route('hotels.edit', hotel.id)" tabindex="-1">
              <icon name="edit" class="block w-6 h-6 fill-green-600" />
            </inertia-link>
            <button
              class="px-4"
              @click.prevent ="showModal(hotel.id)">
...
export default {
  metaInfo: { title: 'Места проживания' },
  layout: Layout,
  components: {
    Icon,
    Pagination,
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('hotels', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
  methods: {
    ...
    destroy() {
        this.$inertia.delete(this.route('hotels.destroy', this.record_id))
    },    
  },
}
</script>

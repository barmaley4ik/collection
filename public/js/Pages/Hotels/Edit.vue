<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      ...
      {{ JSON.parse(this.hotel.festival.name)[defaultlocale] }}
    </h1>
...
export default {
  ...
  data() {
    return {
      ...
      sending: false,
      form: {
        header: JSON.parse(this.hotel.header),
        hotel: JSON.parse(this.hotel.hotel),
        meta_title: JSON.parse(this.hotel.meta_title),
        meta_description: JSON.parse(this.hotel.meta_description),
        images: JSON.parse(this.hotel.images),
        festival_id: this.hotel.festival_id,
      },
  ...
  methods: {
    ...
    submit() {
      this.sending = true
      this.$inertia.put(this.route('hotels.update', this.hotel.id), this.form)
